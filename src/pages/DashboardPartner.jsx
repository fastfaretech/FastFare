...
export default function PartnerDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const p = await apiFetch("/api/v1/partner/dashboard").catch(() => null);
        if (p) {
          setStats(p);
          setTasks(p.tasks || []);
        }
        ...
        <section className="container mx-auto px-6 pb-16 relative z-10">
          <div className="grid lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className={INLINE_BADGE}>
                      ...
                      <span className="text-sm font-semibold text-slate-200">Partner Overview</span>
                    </div>

                    <h2 className="mt-4 text-3xl font-black text-white">Operations</h2>
              ...
              {/* Tasks */}
              <motion.div initial="hidden" animate="show" variants={itemAnim} className={GLASS_CARD_DARK}>
                ...
                <div className="mt-6 space-y-3">
                  {loading ? (
                    ...
                    <div className="text-slate-300">No tasks for today.</div>
                  ) : (
                    tasks.slice(0, 6).map((t) => (
                      ...
                      >
                        <div>
                          <div className="font-medium text-white">{t.awb || t.id}</div>
                          <div className="text-xs text-slate-300">
                            {t.to?.city || t.to?.address}
                          </div>
                        </div>
                        <div className="text-xs text-slate-300">{t.status}</div>
                      </div>
                    ))
                  )}
                </div>
              ...
              {/* Reconciliation */}
              <motion.div initial="hidden" animate="show" variants={itemAnim} className={GLASS_CARD_DARK}>
                <h3 className="text-xl font-bold text-white">Reconciliation</h3>
                ...
                <div className="mt-4 flex gap-2">
                  <button
                    className="px-4 py-2 bg-white/10 rounded-lg text-white"
                    onClick={() => (window.location.href = "/partner/collections")}
                  >
                    Collections
                  </button>
                  <button
                    className="px-4 py-2 bg-cyan-500 rounded-lg text-black"
                    onClick={() => (window.location.href = "/partner/payout")}
                  >
                    Request Payout
                  </button>
                </div>
            ...
            <aside className="space-y-6">
              ...
              <div className={GLASS_CARD_LIGHT}>
                <div className="text-xs text-slate-700">Earnings (month)</div>
              ...
              <div className={GLASS_CARD_LIGHT}>
                ...
                <div className="mt-2 text-black">{stats.fleetName || "No fleet set"}</div>
              </div>

              {/* Quick Actions */}
              <div className={GLASS_CARD_LIGHT}>
                <div className="text-xs text-slate-700">Quick Actions</div>
                <div className="mt-3 grid gap-2">
                  <button
                    className="px-3 py-2 rounded bg-indigo-50 text-slate-900 hover:bg-indigo-100 transition"
                    onClick={() => (window.location.href = "/partner/new-pickup")}
                  >
                    New Pickup
                  </button>
                  <button
                    className="px-3 py-2 rounded bg-indigo-50 text-slate-900 hover:bg-indigo-100 transition"
                    onClick={() => (window.location.href = "/partner/scan")}
                  >
                    Scan & Update
                  </button>
                </div>
              ...
              {/* Driver Section */}
<div className={GLASS_CARD_LIGHT}>
  <div className="text-xs text-slate-700">Driver Section</div>

  <div className="mt-3 grid gap-4">

    <div>
      <label className="block text-xs text-slate-600 mb-1">Driver Name</label>
      <input
    ...
    <button
      ...
      onClick={() => alert("Driver Added! (Backend connection pending)")}
    >
      Add Driver
    </button>
  </div>
